(function() {
  ...
  specCount = function() {
    return page.evaluate(function() {
      return window.reporter.specCount;
    });
  };
  ...
  waitFor = function(test, ready, timeout) {
    ...
    condition = false;
    wait = function() {
      var error, text, lastSpecCount, msg, newSpecCount;

      // Only timeout if jasmine isn't making progress
      newSpecCount = specCount();
      if (lastSpecCount != newSpecCount) {
        lastSpecCount = newSpecCount;
        start = new Date().getTime();
      }
